...
class ChatController extends Controller
{
	...
	public function getAllConversations()
	{
		...
		$users = User::with(['getRole'])->whereHas('roles', function($q){
        			$q->where('name', 'admin')->orWhere('name', 'buyer');
        		 })->get();

		return view('seller.conversations')->with(['conversations' => $conversations, 'users' => $users]);
	}
    ...
    public function getChat($id)
    {
		...
		return view('seller.chat_view')->with(['messages' => $messages, 'conv_with_user' => $conv_with_user]);
    }
    ...
    public function getUnreadConversations()
    {
        ...
        if($conversations)
        {
            ...
            $unread_list = view('seller.renders.unread_conversations_list_render')->with('conversations', $conversations)->render();
