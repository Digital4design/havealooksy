...
class ChatController extends Controller
{
	...
	public function getAllConversations()
	{
		...
		$users = User::with(['getRole'])->whereHas('roles', function($q){
        			$q->where('name', 'admin')->orWhere('name', 'seller');
        		 })->get();

		return view('buyer.conversations')->with(['conversations' => $conversations, 'users' => $users]);
	}
    ...
    public function getChat($id)
    {
		...
		return view('buyer.chat_view')->with(['messages' => $messages, 'conv_with_user' => $conv_with_user]);
    }
    ...
    public function getUnreadConversations()
    {
        ...
        if($conversations)
        {
            ...
            $unread_list = view('buyer.renders.unread_conversations_list_render')->with('conversations', $conversations)->render();
